...
function wci_get_pending_invite_token() {
    ...
    if ( ! empty($_COOKIE['wci_invite_token']) ) {
        return sanitize_text_field(wp_unslash((string) $_COOKIE['wci_invite_token']));
    }
...
function wci_get_brand_settings() {
    $defaults = [
    ...
    $saved = get_option('wci_brand_settings', []);
...
<div style="background:#f4f4f4;padding:24px 0;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Arial,sans-serif;">
  <div style="max-width:640px;margin:0 auto;background:#ffffff;padding:24px;border-radius:8px;">
    ...
    <p style="margin:26px 0 0 0;font-size:14px;line-height:1.5;color:#111;">
      — '.esc_html(wci_get_brand_settings()['site_name']).'<br>
      <a href="'.esc_url(wci_get_brand_settings()['site_url']).'" style="color:#555;text-decoration:none;">'.esc_html(wci_get_brand_settings()['site_url']).'</a>
    </p>
...
function wci_mail_from_name( $name ) {
    return wci_get_brand_settings()['from_name'];
}

add_filter( 'wp_mail_from', 'wci_mail_from' );
function wci_mail_from( $email ) {
    return wci_get_brand_settings()['from_email'];
}
...
function wci_handle_invite_actions() {
    if ( isset($_POST['wci_save_brand_settings']) ) {
        check_admin_referer('wci_brand_settings');
        $settings = [
            'site_name'  => sanitize_text_field(wp_unslash($_POST['wci_brand_site_name'] ?? '')),
            'site_url'   => esc_url_raw(wp_unslash($_POST['wci_brand_site_url'] ?? '')),
            'from_name'  => sanitize_text_field(wp_unslash($_POST['wci_brand_from_name'] ?? '')),
            'from_email' => sanitize_email(wp_unslash($_POST['wci_brand_from_email'] ?? '')),
            'logo_url'   => esc_url_raw(wp_unslash($_POST['wci_brand_logo_url'] ?? '')),
        ];
        ...
        update_option('wci_brand_settings', $settings);
...
$email = sanitize_email(wp_unslash($_POST['wci_invite_email'] ?? ''));
        $days  = max(1, min(365, intval(wp_unslash($_POST['wci_exp_days'] ?? 7))));
    ...
    if ( isset($_POST['wci_send_invite']) && check_admin_referer('wci_invite_send') ) {
        $to      = sanitize_email(wp_unslash($_POST['wci_to'] ?? ''));
        $subject = sanitize_text_field(wp_unslash($_POST['wci_subject'] ?? ''));
        $body    = wp_kses_post(wp_unslash($_POST['wci_body'] ?? ''));
...
function wci_invite_admin_page() {
    ...
    <div class="wrap">
        ...
        <div style="background:#fff;border:1px solid #dcdcde;border-radius:10px;padding:16px;margin:16px 0;">
            ...
            <form method="POST">
                <?php wp_nonce_field('wci_brand_settings'); ?>
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:12px;">
                    <p><label><strong>Site / Brand Name</strong></label><br><input type="text" name="wci_brand_site_name" class="regular-text" value="<?php echo esc_attr(wci_get_brand_settings()['site_name']); ?>"></p>
                    <p><label><strong>Website URL</strong></label><br><input type="url" name="wci_brand_site_url" class="regular-text" value="<?php echo esc_attr(wci_get_brand_settings()['site_url']); ?>"></p>
                    <p><label><strong>From Name</strong></label><br><input type="text" name="wci_brand_from_name" class="regular-text" value="<?php echo esc_attr(wci_get_brand_settings()['from_name']); ?>"></p>
                    <p><label><strong>From Email</strong></label><br><input type="email" name="wci_brand_from_email" class="regular-text" value="<?php echo esc_attr(wci_get_brand_settings()['from_email']); ?>"></p>
                    <p><label><strong>Logo URL</strong></label><br><input type="url" name="wci_brand_logo_url" class="regular-text" value="<?php echo esc_attr(wci_get_brand_settings()['logo_url']); ?>"></p>
                </div>
                <p><button class="button" name="wci_save_brand_settings" value="1">Save Brand Settings</button></p>
            </form>
        </div>
